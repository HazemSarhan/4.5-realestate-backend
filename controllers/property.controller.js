...
export const createProperty = async (req, res) => {
  ...
  if (req.files && req.files.media) {
    const files = Array.isArray(req.files.media)
      ? req.files.media
      : [req.files.media]; // Ensure we handle both single and multiple files
...
export const updateProperty = async (req, res) => {
  ...
  if (amenityIds) {
    parsedAmenityIds = Array.isArray(amenityIds)
      ? amenityIds.map((id) => ({ id: parseInt(id) }))
      : JSON.parse(amenityIds).map((id) => ({ id: parseInt(id) }));
  }

  let parsedFacilityIds = [];
  if (facilityIds) {
    parsedFacilityIds = Array.isArray(facilityIds)
      ? facilityIds.map((id) => ({ id: parseInt(id) }))
      : JSON.parse(facilityIds).map((id) => ({ id: parseInt(id) }));
  }
  ...
  const updatedProperty = await prisma.property.update({
    ...
    data: {
      ...
      additionalFeatures:
        additionalFeatures.length > 0 ? additionalFeatures : undefined,
...
export const deleteProperty = async (req, res) => {
  ...
  res
    ...
    .json({ msg: `Property with id: ${propertyId} deleted.` });
};
